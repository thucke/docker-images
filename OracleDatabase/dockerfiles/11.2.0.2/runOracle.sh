...
function moveFiles {
   ...
   su -p oracle -c "mv $ORACLE_HOME/network/admin/sqlnet.ora $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/"
...
function symLinkFiles {
   ...
   if [ ! -L $ORACLE_HOME/network/admin/sqlnet.ora ]; then
      ln -sf $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/sqlnet.ora $ORACLE_HOME/network/admin/sqlnet.ora
   fi;
...
LISTENER =
   (DESCRIPTION_LIST =
      (DESCRIPTION =
         (ADDRESS_LIST =
            (ADDRESS = (PROTOCOL = IPC)(KEY = EXTPROC_FOR_XE)(QUEUESIZE=50))
...
TRACE_TIMESTAMP = ON
TRACE_FILE_LISTENER = listener_listener.trc
TRACE_LEVEL_LISTENER = OFF
TRACE_FILELEN_LISTENER = 500
TRACE_FILENO_LISTENER = 3
DEFAULT_SERVICE_LISTENER = (XE)" > $ORACLE_HOME/network/admin/listener.ora

# TNS Names.ora
   echo "#####################################################
# Internal connection descritors
...
# Could be used by INIT.ORA as LOCAL_LISTENER using multiple listeners
XE.listener =
   (DESCRIPTION =
      (ADDRESS_LIST =
         (ADDRESS = (PROTOCOL = TCP)(HOST = 0.0.0.0)(PORT = 1521))
...
#####################################################
# Connection strings used to access the database
#####################################################
XE_SHARED =
   (DESCRIPTION =
      (ADDRESS_LIST =
